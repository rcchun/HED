...
def plotResults(inp, images, gt, size,fname,thres):
    ...
    for i in range(0,loss_is):
        ...
        if i==1:
            ...
            # io.imsave(os.path.join('output',fname[0][16:]), np.dstack((TP+FP, FN+FP, np.zeros(TP.shape))))
            # io.imsave(os.path.join('output',fname[0][16:-4]) + '.png', TP+FP)
        if i==2:
...
if __name__ == '__main__':
    ...
    arg_Model = 'train/HED0_v4.pth'
    arg_DataRoot = 'data/bd_vh_03'
    arg_Thres = 120
    ...
    for i, sample in enumerate(testDataloader):
        ...
        print("debug, inp.data[0].shape : ",  inp.data[0].shape)

        inp2 = inp.data[0].permute(1, 2, 0)

        input_img = inp2.cpu().numpy()
        print("debug, input_img.shape: ",input_img.shape)
