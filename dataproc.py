...
class TrainDataset(Dataset):
    def __init__(self, fileNames, rootDir, kernel, thresh, transform=None, target_transform=None):
    ...
    def __getitem__(self, idx):
        ...
        # process and filtering the images
        # inputImage = Image.open(inputName).convert('RGB')
        S_THRESHOLD = self.thresh
        inputImage = Image.open(inputName).convert('L')
        median_filter = ImageFilter.MedianFilter(self.kernel)
        median_image = inputImage.filter(median_filter)
        inputImage = np.array(median_image)
        inputImage[inputImage >= S_THRESHOLD] = 255
        inputImage = np.stack((inputImage,) * 3, axis=-1)
        io.imsave(inputName.split('.jpg')[0]+'.jpeg', inputImage)
...
class TrainDatasetNonFilter(Dataset):
...
class TestDatasetNonFilter(Dataset):
