...
def plotResults2(inp, images, gt, size,fname,thres):
    ...
    for i in range(0,gts):
        ...
        if i==1:
            ...
            #plt.imshow(gt)
    io.imsave(os.path.join('output/pred',fname[0][16:-4]) + '_mask.png', images[0])  
    ...
    for j in range(len(contours)):
        if len(contours[j]) > 1 and cv2.contourArea(contours[j]) > 200:
            area_list.append(cv2.contourArea(contours[j]))
            x0, y0 = zip(*np.squeeze(contours[j]))
            plt.plot(x0, y0, c="b", linewidth = 1.0)
            plt.imshow(inp)
            plt.imshow(mask, alpha=0.3)
            plt.savefig(os.path.join('output/pred',fname[0][16:-4]) + '_maskContour.png', bbox_inches='tight', pad_inches=0, dpi=300)
            plt.clf()
    
    plt.imshow(inp)
    plt.imshow(mask, alpha=0.3)
    plt.axis('off')
    plt.savefig(os.path.join('output/pred',fname[0][16:-4]) + '_maskoverlay.png', bbox_inches='tight', pad_inches=0, dpi=300)
    plt.clf()
...
if __name__ == '__main__':
    ...
    arg_Model = 'train/HED0_v4_retrain.pth'
    arg_DataRoot = 'data/bbox_vh_test'
    arg_Thres = 120
